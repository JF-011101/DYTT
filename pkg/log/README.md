...
`jf-011101/log` 是一个基于 `zap` 包封装的日志包。除了实现 `Go` 日志包的基本功能外，还实现了很多高级功能，`jf-011101/log`具有如下特性：
...
import (
    "github.com/jf-011101/iam/pkg/log"
)
...
上述代码使用 `jf-011101/log` 包默认的全局 `logger`，分别在 `Debug` 、`Info` 和 `Warn` 级别打印了一条日志。
